 ...
 * resolves to for the 'enclave' app. Only used when the host hasn't supplied
 * context.utils.paths (older Enclave builds before list-providers / paths
 * plumbing landed).
 */
function resolveUserData() {
  const home = os.homedir();
  switch (process.platform) {
    case 'darwin': return path.join(home, 'Library', 'Application Support', 'enclave', 'enclave-data');
    case 'win32': return path.join(process.env.APPDATA || path.join(home, 'AppData', 'Roaming'), 'enclave', 'enclave-data');
    default: return path.join(process.env.XDG_CONFIG_HOME || path.join(home, '.config'), 'enclave', 'enclave-data');
  }
}
...
class AgentConverter {
  constructor(context) {
    ...
    // userData location for older Enclave hosts that don't expose it.
    this.avatarBasePath =
  ...
  convert(card, options = {}) {
    ...
    // Enclave's `avatar://` protocol resolves filenames against
