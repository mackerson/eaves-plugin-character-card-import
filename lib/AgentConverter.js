/**
 * Agent Converter
 * Converts character card data to Eaves agent format
 */
...
/**
 * Per-platform Electron userData fallback. Mirrors what app.getPath('userData')
 * resolves to for the 'eaves' app. Only used when the host hasn't supplied
 * context.utils.paths (older Eaves builds before list-providers / paths
 * plumbing landed).
 */
function resolveUserData() {
  const home = os.homedir();
  switch (process.platform) {
    case 'darwin': return path.join(home, 'Library', 'Application Support', 'eaves');
    case 'win32': return path.join(process.env.APPDATA || path.join(home, 'AppData', 'Roaming'), 'eaves');
    default: return path.join(process.env.XDG_CONFIG_HOME || path.join(home, '.config'), 'eaves');
  }
}
...
class AgentConverter {
  constructor(context) {
    ...
    // Worker-thread plugins can't `require('electron').app.getPath(...)`.
    // Prefer the host-supplied path (context.utils.paths.avatars set up by
    // worker-entry from workerData), then fall back to the per-platform
    // userData location for older Eaves hosts that don't expose it.
    this.avatarBasePath =
      context.utils?.paths?.avatars
      || path.join(resolveUserData(), 'avatars');
  ...
  /**
   * Convert a character card to an Eaves agent
  ...
  convert(card, options = {}) {
    ...
    // Handle avatar if source file provided. Store ONLY the filename —
    // Eaves's `avatar://` protocol resolves filenames against
    // userData/avatars, and persisting an absolute path makes the avatar
    // unreachable on restore-to-another-machine.
    if (card._source && options.importAvatar !== false) {
      try {
        const avatarPath = this.copyAvatar(card._source, data.name);
        if (avatarPath) {
          agent.avatar = path.basename(avatarPath);
        }
      } catch (e) {
  ...
  /**
   * Replace template variables with Eaves format
